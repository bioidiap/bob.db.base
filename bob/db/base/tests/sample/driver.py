...
def _checkfiles(args):
  ...
  bad = [f for f in r if not os.path.exists(f.make_path(directory=args.directory, extension=args.extension))]
...
class Interface(AbstractInterface):
  ...
  def add_commands(self, parser):
    ...
    dump_parser.add_argument('-d', '--directory', help="if given, this path will be prepended to every entry returned [default: <internal>]")
    dump_parser.add_argument('-e', '--extension', default='.png', help="if given, this extension will be appended to every entry returned.")
    dump_parser.add_argument('-g', '--group', help="if given, this value will limit the output files to those belonging to a particular group.", choices=('train', 'test'))
    ...
    check_parser.add_argument('-d', '--directory', help="the path to the root directory to use [default: <internal>]")
    check_parser.add_argument('-e', '--extension', default=".png", help="the extension appended to every sample [default: %(default)s]")
    check_parser.add_argument('-g', '--group', help="if given, this value will limit the output files to those belonging to a particular group.", choices=('train', 'test'))
    check_parser.add_argument('--self-test', dest="selftest", action='store_true', help=SUPPRESS)
